...
class Trainer(object):
    def __init__(self, thread_index, global_network, initial_learning_rate,
        ...
        self.local_network = UnrealModel(
            self.action_size, thread_index, use_pixel_change, use_value_replay,
            use_reward_prediction, pixel_change_lambda, entropy_beta, device)
        self.local_network.prepare_loss()

        self.apply_gradients = grad_applier.minimize_local(
            self.local_network.total_loss,
            global_network.get_vars(), self.local_network.get_vars())
    ...
    def _process_base(self, sess, global_t, summary_writer, summary_op,
        ...
        R = 0.0
        if not terminal_end:
            R = self.local_network.run_base_value(sess, new_state,
                                                  frame.get_action_reward(
                                                      self.action_size))
        ...
        return batch_si, last_action_rewards, batch_a, \
            batch_adv, batch_R, start_lstm_state
    ...
    def process(self, sess, global_t, summary_writer, summary_op, score_input):
        ...
        feed_dict = {
            ...
            batch_R,
            self.local_network.base_initial_lstm_state:
            start_lstm_state,
    # [common]
        ...
        if self.use_pixel_change:
            ...
            pc_feed_dict = {
                self.local_network.pc_input:
                batch_pc_si,
                self.local_network.pc_last_action_reward_input:
                batch_pc_last_action_reward,
                self.local_network.pc_a:
                batch_pc_a,
                self.local_network.pc_r:
                batch_pc_R
            }
        ...
        if self.use_value_replay:
            ...
            vr_feed_dict = {
                self.local_network.vr_input:
                batch_vr_si,
                self.local_network.vr_last_action_reward_input:
                batch_vr_last_action_reward,
                self.local_network.vr_r:
                batch_vr_R
            }

...
class RMSPropApplier(object):
    ...
    def _prepare(self):
        ...
        self._momentum_tensor = tf.convert_to_tensor(
            self._momentum, name="momentum")
        self._epsilon_tensor = tf.convert_to_tensor(
            self._epsilon, name="epsilon")
    ...
    def _apply_gradients(self, global_var_list, local_grad_list, name=None):
        ...
        local_grad_list, _ = tf.clip_by_global_norm(local_grad_list,
                                                    self._clip_norm)

        with tf.name_scope(name, self._name, []) as name:
            self._prepare()
            for var, grad in zip(global_var_list, local_grad_list):
                with tf.name_scope("update_" + var.op.name), tf.device(
                        var.device):

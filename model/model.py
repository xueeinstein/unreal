...
def conv_initializer(kernel_width,
                     kernel_height,
                     input_channels,
                     dtype=tf.float32):
    def _initializer(shape, dtype=dtype, partition_info=None):
...
class UnrealModel(object):
  ...
  """
    ...
    def _create_base_network(self):
        ...
        self.base_last_action_reward_input = tf.placeholder(
            "float", [None, self._action_size + 1])
        ...
        self.base_initial_lstm_state = tf.contrib.rnn.LSTMStateTuple(
            self.base_initial_lstm_state0, self.base_initial_lstm_state1)
        ...
        self.base_pi = self._base_policy_layer(
            self.base_lstm_outputs)    # policy output
        self.base_v = self._base_value_layer(
            self.base_lstm_outputs)    # value output
    ...
    def _base_conv_layers(self, state_input, reuse=False):
        with tf.variable_scope("base_conv", reuse=reuse) as scope:
            ...
            W_conv2, b_conv2 = self._conv_variable([4, 4, 16, 32],
                                                   "base_conv2")

            # Nodes
            h_conv1 = tf.nn.relu(
                self._conv2d(state_input, W_conv1, 4) + b_conv1)    # stride=4
            h_conv2 = tf.nn.relu(
                self._conv2d(h_conv1, W_conv2, 2) + b_conv2)    # stride=2
            return h_conv2

    def _base_lstm_layer(self,
                         conv_output,
                         last_action_reward_input,
                         initial_state_input,
                         reuse=False):
        with tf.variable_scope("base_lstm", reuse=reuse) as scope:
            ...
            # (-1,9,9,32) -> (-1,2592)
            conv_output_fc = tf.nn.relu(
                tf.matmul(conv_output_flat, W_fc1) + b_fc1)
            # (unroll_step, 256)

            step_size = tf.shape(conv_output_fc)[:1]

            lstm_input = tf.concat([conv_output_fc, last_action_reward_input],
                                   1)
            # (unroll_step, 256+action_size+1)

            lstm_input_reshaped = tf.reshape(
                lstm_input, [1, -1, 256 + self._action_size + 1])
    ...
    def _base_policy_layer(self, lstm_outputs, reuse=False):
        with tf.variable_scope("base_policy", reuse=reuse) as scope:
            ...
            W_fc_p, b_fc_p = self._fc_variable([256, self._action_size],
                                               "base_fc_p")
            # Policy (output)
    ...
    def _create_pc_network(self):
        ...
        # Last action and reward
        self.pc_last_action_reward_input = tf.placeholder(
            "float", [None, self._action_size + 1])
        ...
        pc_lstm_outputs, _ = self._base_lstm_layer(
            pc_conv_output,
    ...
    def _create_pc_network_for_display(self):
        self.pc_q_disp, self.pc_q_max_disp = self._pc_deconv_layers(
            self.base_lstm_outputs, reuse=True)

    def _pc_deconv_layers(self, lstm_outputs, reuse=False):
        with tf.variable_scope("pc_deconv", reuse=reuse) as scope:
            # (Spatial map was written as 7x7x32, but here 9x9x32 is used to
            # get 20x20 deconv result?)
            # State (image input for pixel change)
            W_pc_fc1, b_pc_fc1 = self._fc_variable([256, 9 * 9 * 32], "pc_fc1")

            W_pc_deconv_v, b_pc_deconv_v = self._conv_variable(
                [4, 4, 1, 32], "pc_deconv_v", deconv=True)
            W_pc_deconv_a, b_pc_deconv_a = self._conv_variable(
                [4, 4, self._action_size, 32], "pc_deconv_a", deconv=True)
            ...
            h_pc_deconv_v = tf.nn.relu(
                self._deconv2d(h_pc_fc1_reshaped, W_pc_deconv_v, 9, 9, 2) +
                b_pc_deconv_v)
            h_pc_deconv_a = tf.nn.relu(
                self._deconv2d(h_pc_fc1_reshaped, W_pc_deconv_a, 9, 9, 2) +
                b_pc_deconv_a)
            # Advantage mean
            ...
            # Max Q
            pc_q_max = tf.reduce_max(
                pc_q, reduction_indices=3, keep_dims=False)
            # (-1, 20, 20)
    ...
    def _create_vr_network(self):
        ...
        # Last action and reward
        self.vr_last_action_reward_input = tf.placeholder(
            "float", [None, self._action_size + 1])
        ...
        vr_lstm_outputs, _ = self._base_lstm_layer(
            vr_conv_output,
    ...
    def _create_rp_network(self):
        ...
        rp_conv_output_reshaped = tf.reshape(rp_conv_output,
                                             [1, 9 * 9 * 32 * 3])
        ...
        self.rp_c = tf.nn.softmax(
            tf.matmul(rp_conv_output_reshaped, W_fc1) + b_fc1)
        # (1,3)
    ...
    def _base_loss(self):
        ...
        policy_loss = -tf.reduce_sum(
            tf.reduce_sum(
                tf.multiply(log_pi, self.base_a), reduction_indices=1) *
    ...
    def reset_state(self):
        self.base_lstm_state_out = tf.contrib.rnn.LSTMStateTuple(
    ...
    def run_base_policy_and_value(self, sess, s_t, last_action_reward):
        ...
        pi_out, v_out, self.base_lstm_state_out = sess.run(
            [self.base_pi, self.base_v, self.base_lstm_state],
            feed_dict={
                self.base_input: [s_t],
    ...
    def run_base_policy_value_pc_q(self, sess, s_t, last_action_reward):
        ...
        pi_out, v_out, self.base_lstm_state_out, q_disp_out, q_max_disp_out = \
            sess.run([self.base_pi, self.base_v, self.base_lstm_state,
                      self.pc_q_disp, self.pc_q_max_disp],
                     feed_dict = feed_dict)
    ...
    def run_base_value(self, sess, s_t, last_action_reward):
        # This run_bae_value() is used for calculating V for bootstrapping at
        # the end of LOCAL_T_MAX time step sequence.
        # When next sequence starts, V will be calculated again with the same
        # state using updated network weights,
        # so we don't update LSTM state here.
        v_out, _ = sess.run(
            [self.base_v, self.base_lstm_state],
            feed_dict={
                self.base_input: [s_t],
                self.base_last_action_reward_input: [last_action_reward],
                self.base_initial_lstm_state0: self.base_lstm_state_out[0],
    ...
    def run_rp_c(self, sess, s_t):
        # For display tool
        rp_c_out = sess.run(self.rp_c, feed_dict={self.rp_input: s_t})
        return rp_c_out[0]
    ...
    def _deconv2d(self, x, W, input_width, input_height, stride):
        ...
        out_height, out_width = self._get2d_deconv_output_size(
            input_height, input_width, filter_height, filter_width, stride,
            'VALID')
